...
|---|---|---|
| `widget` | `widget/` (`t()` in `widget/src/i18n.ts`; sliced per locale into `dist/locales/<code>.js`) | W-A |
| `public` | public board / roadmap / tenant-host landing (i18next) | W-B |
| `admin` | admin console (i18next) | W-B (bootstrap), W-D (extraction) |
| `email` | `crates/feedbackmonk-i18n` (`include_str!`) — confirmation / status / reply / account emails | W-C |
...
3. **Keys are dotted, namespaced, camelCase leaves**: `widget.form.subject`, `email.status.subject`,
   ...
   first segment equals the file's namespace.
...
| `check-gaps.py` | MISSING + DRIFTED report, `--json`, `--strict` | 1 (W-T) |
| `translate.py` | DeepL pass with quota preflight and round-trip safety; owner-only | 1 (W-T) |
| `validate.py` | structural + quality validation (nine classes) | 1 (W-T) |
| `complete-plurals.py` | CLDR plural-category completion | 1 (W-T) |
