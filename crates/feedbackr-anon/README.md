...
- Owner module: crates/feedbackr-anon/
...
- Mint opaque anon cookies (16 random bytes, base64url-no-pad) when a
  request arrives with no `X-Feedbackr-Anon-Cookie` header.
...
```rust
use feedbackr_anon::{AnonGate, RateLimitError, ANON_COOKIE_HEADER};
...
let cookie: &str = /* X-Feedbackr-Anon-Cookie header value or AnonGate::mint_cookie() */;
...
`HASH_DOMAIN_PREFIX = b"feedbackr-anon-v1"` — increment the version suffix
