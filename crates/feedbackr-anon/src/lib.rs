//! `feedbackr-anon` -- anonymous-mode rate-limit + cookie dedup for the
...
//!   `FEEDBACKR_ANON_RATE_LIMIT_PER_HOUR`).
//! - `AnonGate::token_hash` -- BLAKE3 of
//!   `b"feedbackr-anon-v1" || ip || 0 || cookie || 0 || project_id`. Version
//!   prefix enables hash-domain rotation without ambiguity.
//! - `AnonGate::mint_cookie` -- 16 random bytes, base64url-no-pad. Used by
//!   the handler when no `X-Feedbackr-Anon-Cookie` header arrives.
//!
...
pub const HASH_DOMAIN_PREFIX: &[u8] = b"feedbackr-anon-v1";

/// Default per-(anon_hash, project) submissions per hour. Overridable by
/// `FEEDBACKR_ANON_RATE_LIMIT_PER_HOUR` env var (handler-level wiring).
...
pub const ANON_COOKIE_HEADER: &str = "X-Feedbackr-Anon-Cookie";
...
impl AnonGate {
    ...
    /// `X-Feedbackr-Anon-Cookie` header is present on the request.
    #[must_use]
