...
| `src/feedback.rs` | `FeedbackRepo` trait + `SqlxFeedbackRepo` impl. P0: `submit_authenticated`, `submit_anonymous`, `list_recent`. **P1 Stage 1 additions**: `list_for_admin(scope, status_filter, limit, offset)` + `get_with_history(scope, feedback_id)` (Contract C6 backing). **P1 Stage 2 addition**: `update_status_in_executor(scope, conn, feedback_id, new_status)` — same-txn `feedback.status` UPDATE companion to `feedback_status_history::append_in_executor` (Contract C6 Hard Invariant #4). **P5a additions**: `Sentiment` field in submit methods + `sentiment_trend(scope, days)` aggregate for the dashboard. Also defines `FeedbackListItem` and `StatusHistoryRow` value types. **FR-FBR-30 additions** (migration 00019): the translate-after-accept worklist methods `claim_pending_translations` / `set_translation` / `mark_translation_skipped` / `mark_translation_failed` (tenant-agnostic transport keyed on `feedback.id`; allow-listed pre-auth exceptions), the `PendingTranslation` value type, and the process-global `TranslationFlag` enablement gate the submit methods read to stamp `translation_status='pending'`. The analyst read `list_member_bodies_for_cluster` now reads `coalesce(NULLIF(body_translated,''), body)`. Worklist tests live in `tests/translation_repo.rs`. |
| `src/board_votes.rs` | **PF-BOARD-VOTING-01 (migration 00018, Contract C30).** `BoardVoteRepo` trait + `SqlxBoardVoteRepo` impl backing public feedback-board voting — the board sibling of `roadmap_votes`, keyed on `feedback_id` (NOT a generalization; `roadmap_votes` is left untouched per DEC-FBR-IMPL-21). `cast` (409 on duplicate `(feedback_id, voter_id)`), `retract` (window-checked `RetractOutcome`), `vote_count_for_feedback`, `has_voted` — all `&ProjectScope`-first; reuses `RoadmapVoterMode` + `RetractOutcome`. Defines the `BoardVote` value type. Board `vote_count` is a **direct SQL aggregate** (correlated count in `feedback.rs::list_public_board`/`get_public_board_item`, D1) — no cache. The moderation gate (D2) lives one layer up: the API vote handlers resolve an approved-only `feedback.id` via `FeedbackRepo::resolve_approved_board_feedback_id` before calling `cast`. 6 sqlx::test (cast/dup-409/cross-tenant-404/retract-window). |
...
| `src/solicitations.rs` | **P5a**: `SolicitationRepo` trait + `SqlxSolicitationRepo` impl. Manages `solicitations` table — per-project question prompts for feedback submission enrichment (FR-FBR-26). Methods: `create(scope, name, prompt, enabled)` + `list_enabled_for_project(scope)` + `update(scope, id, ...)`. All `&ProjectScope`-first. Defines `Solicitation` value type. 3 sqlx::test cases including cross-tenant negatives. |
...
| `src/password_resets.rs` | **Scrutiny P1-1 (migration 00024).** `PasswordResetRepo` trait + `PasswordReset` value type + `SqlxPasswordResetRepo` impl backing the admin password-reset flow. `create(&TenantScope, token_hash, expires_at)` / `redeem(token_hash)` / `mark_used(&TenantScope, token_hash)`. Sibling of `email_verifications` with two differences: the PK is `token_hash` (**sha256-at-rest**, hashed in the API layer — a table read can't be replayed to seize an account) and the TTL is short (default 1h). **`redeem` is pre-auth allow-listed** (the reset token IS the credential; the tenant forgot their password). 4 sqlx::test (round-trip, unknown-digest, mark-used, cascade-delete). |
| `src/tenants.rs` (P1-1 session-revocation extension) | **Scrutiny P1-1 (migration 00023).** `TenantRepo` gains `bump_session_epoch(&TenantScope)` (increments `tenants.session_epoch` — the admin-session revocation kill switch folded into the signed cookie) + `set_password(&TenantScope, hash)` (password-reset confirm writer). The `Tenant` value type (`feedbackmonk-core`) gains `session_epoch: i32`; `create` / `find_by_email` / `get` now select it. 2 net-new sqlx::test (epoch monotonic increment + cross-tenant isolation; set_password replaces hash). |
...
| `src/tenants.rs` (post-v1 override extension) | **DEC-FBR-IMPL-11/12 (migration 00012).** `TenantRepo` trait gains `set_tier(&TenantScope, Tier)`, `get_widget_brand_override(&TenantScope) -> WidgetBrandOverride`, `set_widget_brand_override(&TenantScope, &WidgetBrandOverride)`. New `WidgetBrandOverride` value type carries the five nullable override facets (`footer_text_override`, `footer_url`, `theme`, `primary_color`, `logo_url`). `get_widget_brand` now layers the per-tenant override **above** the tier-derived default: a non-null `footer_text_override` supersedes the FR-FBR-14 tier footer (`""` suppresses, non-empty customizes); null facets fall through to tier/CSS defaults. The tier-aware base (above) is unchanged — the override is an additional resolution layer, not a rewrite. Written only via the `OpsAuth`-guarded ops endpoint. Net-new sqlx::test cover override round-trip + null-fallthrough + footer-suppression-vs-tier-default. |
