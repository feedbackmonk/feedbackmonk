...
| `src/models.rs` | Plain `struct` / `enum` records mirroring the P0 schema. Includes 3 unit tests over `FeedbackKind` parse round-trips and `Feedback` field defaults. **P5a extension**: added `Sentiment` enum (`Positive | Neutral | Negative`). |
