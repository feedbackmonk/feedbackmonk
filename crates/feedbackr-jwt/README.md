...
- Purpose: EdDSA-only JWT verifier for Feedbackr's submission API. Pure
  computation crate — no async, no I/O, no DB. The end-user JWT is the
  ONLY identity Feedbackr ever has for an auth-mode submitter (DEC-FBR-04).
- Owner module: crates/feedbackr-jwt/
- Read first: this README + Contract C2 in
  docs/planning/plans/20260513T185711-feedbackr-v1-build-arc.md
-->
...
```rust
use feedbackr_jwt::{verify, verify_with_leeway, JwtError, VerifiedClaims};
use feedbackr_core::SigningKey;
use uuid::Uuid;
...
6. **NEVER use `verify` to validate non-end-user tokens.** Per DEC-FBR-04
   ...
   long-lived bearer tokens for Feedbackr-issued identities anywhere —
   this verifier is single-purpose.

## 5. Relationships & Dependencies

- **Consumed by**: `crates/feedbackr-api/src/handlers/feedback.rs`
...
- **Crate deps**: `ed25519-dalek` (signature verification), `base64`
  ...
  `feedbackr-core` (SigningKey type).
...
- **EdDSA only, not RSA / ECDSA.** Single algorithm minimizes verifier
  surface area. Customers integrating with Feedbackr's submission API
