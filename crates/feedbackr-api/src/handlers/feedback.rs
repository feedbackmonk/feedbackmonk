...
//!   verified via `feedbackr_jwt::verify` against the project's active
...
//!   `X-Feedbackr-Anon-Cookie` cookie, computes `token_hash(ip, cookie,
...
use feedbackr_anon::{AnonGate, ANON_COOKIE_HEADER};
use feedbackr_core::FeedbackKind;
use feedbackr_jwt::{verify_with_leeway as jwt_verify_with_leeway, JwtError, VerifiedClaims};
...
async fn submit_authenticated_path(
    state: &AppState,
    project_scope: &feedbackr_repository::ProjectScope,
    token: &str,
    project_id: Uuid,
    body: &str,
    kind: FeedbackKind,
...
async fn submit_anonymous_path(
    ...
    match state.anon_gate.check(&token_hash, project_id) {
        Ok(_) => {}
        Err(feedbackr_anon::RateLimitError::Exceeded {
            retry_after_seconds,
...
/// Read the anon cookie from `X-Feedbackr-Anon-Cookie` request header. If
