...
- Purpose: HTTP request handlers for the feedbackr-api binary. Each file
...
- Owner module: crates/feedbackr-api/src/handlers/
...
| `verify_email.rs` | `POST /api/v1/verify-email` | Redeem verify-email token; on success, mark tenant verified + issue `feedbackr_session` cookie (Contract C11). |
...
```rust
use feedbackr_api::{worker_a_router, submission_router, admin_feedback_routes};
...
- **Crate deps within feedbackr-api**: `state::AppState`, `error::ApiError`,
  `auth::AdminSession` + `auth::issue_session_cookie`,
  `email::{Mailer, EmailNotifier, EmailKind, EmailContext}`.
- **External crate deps**: `feedbackr-core` (FeedbackId / FeedbackKind /
  FeedbackStatus / `legal_transitions_from`), `feedbackr-repository`
  (every repo trait + scope newtypes + `EmailTenantBrand`),
  `feedbackr-jwt` (`verify_with_leeway`, `JwtError`), `feedbackr-anon`
...
- **Submission handler exposes its anon-cookie minting via
  ...
  `X-Feedbackr-Anon-Cookie` header arrives, and returns it via
