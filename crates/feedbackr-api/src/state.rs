...
/// here once the `feedbackr-jwt` and `feedbackr-anon` crates land. Construction
...
pub struct AppState {
    ...
    /// `feedbackr_anon::DEFAULT_RATE_LIMIT_PER_HOUR = 10`; override via
    ...
    /// `FEEDBACKR_JWT_LEEWAY_SECONDS` at startup; default 5s. Only `iat` is
