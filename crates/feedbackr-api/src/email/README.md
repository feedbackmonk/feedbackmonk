...
- Owner module: crates/feedbackr-api/src/email/
...
```rust
use feedbackr_api::email::{render_status_change, StatusChangeContext, RenderedEmail};
use feedbackr_core::FeedbackStatus;
...
```rust
use feedbackr_api::email::{EmailKind, EmailContext, EmailNotifier};
...
```rust
use feedbackr_api::email::is_submitter_visible_transition;
...
- **Reads** `EmailTenantBrand` from `feedbackr_repository::TenantRepo::get_brand`
  (Contract C10; brand columns added by migration 00005).
- **Reads** `FeedbackStatus` + `FeedbackId` from `feedbackr_core`.
- **Inherits** PII scrubbing from `feedbackr_tracing::install_global_subscriber`
