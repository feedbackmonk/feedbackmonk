...
- Owner module: crates/feedbackr-api/src/auth/
...
endpoint behind the `feedbackr_session` cookie (Contract C11):
...
| Property | Value |
|---|---|
| Name | `feedbackr_session` (the `SESSION_COOKIE_NAME` constant) |
...
1. **`feedbackr_session` is the ONLY admin session cookie name.** The
   P1 plan referenced `feedbackr_admin_session`; the P0 implementation
   ships `feedbackr_session` (Contract C11 reconciles). Worker A + B +
   e2e scripts + tests all use `feedbackr_session` exactly.
...
- **Consumed by**: every admin endpoint in
  `crates/feedbackr-api/src/handlers/` (signup completes the loop by
...
- **Crate deps**: `argon2`, `axum`, `axum-extra` (CookieJar), `base64`,
  `chrono`, `hmac`, `sha2`, `subtle`, `uuid`, `feedbackr-repository`
  (TenantScope).
- **Secret source**: `state.session_secret: Arc<[u8; 32]>`. Loaded from
  `FEEDBACKR_SESSION_SECRET` env var (64 hex chars → 32 raw bytes) at
