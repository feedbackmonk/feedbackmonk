...
//!
//! Drives the actual `Router` produced by `feedbackr_api::router::router` with
...
use feedbackr_api::router::router;
...
fn build_test_state(pool: &PgPool, mailer: Arc<RecordingMailer>) -> (AppState, Arc<RecordingMailer>) {
    let state = AppState {
        ...
        health: feedbackr_repository::SqlxHealthCheck::new(pool.clone()),
    };
    (state, mailer)
}
