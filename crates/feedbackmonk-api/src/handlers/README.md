...
| `admin_feedback.rs` | `GET /api/v1/admin/feedback` + `GET /api/v1/admin/feedback/:id` + `POST /api/v1/admin/feedback/:id/transition` + `POST /api/v1/admin/feedback/:id/reply` + `GET /api/v1/admin/feedback/search` | Admin status-workflow + reply + full-text search endpoints (FR-FBR-07/08, Contracts C7 + C8). Search accepts the same optional `status` filter as the list endpoint (so the admin UI's status pills compose with a query) and uses `websearch_to_tsquery` over the `00011`-migration tsvector+GIN index (GitCellar parity gap #3). |
...
| `board.rs` | `GET /api/v1/projects/:id/board` + `GET .../board/items/:short_code` + `POST`/`DELETE .../board/items/:short_code/vote` | Public approved-only feedback board read (Contract C29) + voting (Contract C30, PF-BOARD-VOTING-01). CORS-exposed. The vote handlers enforce the **moderation gate (D2)**: `open_for_submission` → `ensure_board_enabled` → `resolve_approved_board_feedback_id` (approved-only SQL resolution) BEFORE any `board_votes` write, so a vote/retract on a pending/rejected/board-disabled item 404s like the read path (no existence oracle). Static-verified by `public-board-moderation-gate` Probe B (v1.1.0). |
| `voting_common.rs` | — (shared helper module) | THE voter-resolution chokepoint for BOTH the roadmap (C15) and board (C30) voting handlers — extracted from `roadmap.rs` (pure move). `resolve_voter` / `resolve_voter_no_rate_limit` (JWT `sub` or `hex(AnonGate::token_hash(ip,cookie,project_id))`) + the shared 401/403/404/409/429 vote-error response builders. Implements the "anon hash is a canonical chokepoint, never parallel-implemented" rule (migration 00007/00018 inv #2). |
...
| `capabilities.rs` | `GET /api/v1/admin/capabilities` | **P5a**: Project capabilities + solicitations list (FR-FBR-26). Returns enabled `Solicitation` questions + sentiment-feature availability per tier. Admin-gated, scope-bound. |
| `solicitation.rs` | `POST /api/v1/admin/solicitations` + `PATCH /api/v1/admin/solicitations/:id` + `DELETE /api/v1/admin/solicitations/:id` | **P5a**: Solicitation CRUD handlers for the admin settings surface (FR-FBR-26). Create question prompts, toggle enabled, delete. All admin-gated and `&ProjectScope`-first. |
