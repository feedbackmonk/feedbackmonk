...
- Compose the process-wide `tracing-subscriber` Subscriber for the
  `feedbackmonk-api` binary (and any future binaries in the workspace).
...
```rust
use feedbackmonk_tracing::{install_global_subscriber, LogFormat, LogLevel};

fn main() -> anyhow::Result<()> {
    install_global_subscriber(LogLevel::Info, LogFormat::Json)?;
    tracing::info!("feedbackmonk-api listening on 14304");
    Ok(())
}
```
...
```rust
use feedbackmonk_tracing::SharedBufferScrubbing;
...
- **Consumed by**: `feedbackmonk-api` (via `bin/feedbackmonk-api/src/main.rs`,
...
- **`SharedBufferScrubbing` is `pub` (not `pub(crate)`)**. Integration tests
  ...
  `feedbackmonk-api` integration tests in P1 Stage 2) install per-test
