...
### DEC-FBR-IMPL-25: Multilingual feedback — store-both + async translate-after-accept; English canonical target; FTS indexes the translation; universal (not tier-gated); lazy backfill (FR-FBR-30)

**Resolved**: 2026-06-21 (multilingual feedback ideation — GitCellar many-language userbase). Ratifies ideation decisions D-XLATE-1/2/3/5 + the two ideation open-question resolutions (FTS indexing, tier interaction, backfill).

**Decision**: Add a translation pipeline for non-English feedback with these load-bearing choices:
- **Translate, don't just store** (D-XLATE-1): non-English feedback is translated to a canonical language so admins AND machine consumers (FR-FBR-28 sentiment, the P5 agentic loop / clustering) get understandable English; otherwise those English-assuming consumers degrade silently.
- **Store-both** (D-XLATE-2): the verbatim `body` is **never overwritten** — a new nullable `feedback.body_translated` + `feedback.source_lang` (detected language) sit alongside it. This is *forced* by the Q24 invariant (roadmap promotion reads `body` verbatim, no attribution) and gives auditability.
- **Async after-accept timing** (D-XLATE-3): translation runs as a background job AFTER the feedback row is accepted — **never synchronously on the public submit path**. Synchronous would add latency to, and a hard provider-uptime dependency on, the rate-limited public widget endpoint. Consumers read `body_translated` with **fallback to `body`** when the translation is absent (not yet run, disabled, or already-English).
- **English canonical target for v1** (D-XLATE-5): the machine consumers genuinely need English (their prompts/models assume it). A configurable per-project *primary/admin* target language is explicitly **deferred** to a future enhancement.
- **FTS indexes the translation** (ideation open-Q resolved): `body_tsv` (migration 00011) is `GENERATED ALWAYS AS (to_tsvector('english', body))` — hardcoded to the `'english'` config, so it currently mis-stems non-English bodies. Repoint full-text search at the translated-English text so the existing English-config index becomes *correct* for non-English rows; the verbatim `body` stays stored for display + Q24. Clustering / the P5b analyst read `body_translated` with original fallback.
- **Universal, not tier-gated** (ideation open-Q resolved): translation is available on all tiers including Free and self-host. It is a correctness/quality feature, not a premium add-on; self-hosters supply their own provider key, so SaaS provider cost is the only lever and is better handled operationally (or as a future tier knob) than by gating the feature.
- **Lazy backfill** (ideation open-Q resolved): translation applies only to feedback accepted after the feature is enabled. Pre-existing rows keep a NULL `body_translated` and fall back to the original. No migration-time batch job, no up-front bulk provider spend; a manual admin-triggered backfill can be added later.

**Rationale**: The async + store-both + fallback shape is the minimal-blast-radius design that (a) keeps the public submit path's latency and uptime independent of the translation provider, (b) preserves the Q24 verbatim invariant by construction, and (c) lets every existing consumer keep working unchanged (NULL translation → read original). Indexing the translation turns the pre-existing `'english'`-config FTS from "tolerated mis-stemming" into correctness at zero added index machinery. Universal + lazy keeps v1 scope tight and avoids both tier-enforcement wiring and a large enablement-time provider bill.

**Scope** (planned): migration `00019` (nullable `body_translated` + `source_lang`; FTS source change), async translate-after-accept job, consumer wiring in `feedbackmonk-core`/`feedbackmonk-repository` (sentiment + clustering read translated-with-fallback) + `feedbackmonk-runner` analyst. Provider abstraction is DEC-FBR-IMPL-26.

**Alternatives considered**: *Translate-on-read* — recomputes per view, couples read latency to the provider, and can't be indexed for FTS (rejected; store-both chosen). *Synchronous at-submit translation* — adds latency + a hard provider dependency to the public rate-limited path (rejected). *Overwrite `body` with the translation* — violates Q24 (rejected). *One-shot backfill at enablement* — complete coverage but adds a batch job + up-front bulk provider cost proportional to existing volume (rejected for v1; lazy chosen). *Tier-gate translation* — adds tier-enforcement wiring (Contract C19) and a self-host carve-out for a correctness feature (rejected; universal chosen).

---

### DEC-FBR-IMPL-26: Translation provider is pluggable and defaults OFF; v1 ships DeepL (cloud, EU) + off; a local no-egress engine is future; egress is a conscious, disclosed choice (FR-FBR-30)

**Resolved**: 2026-06-21 (multilingual feedback ideation). Ratifies ideation decision D-XLATE-4 + the legal/compliance analysis.

**Decision**: The translation provider is a **pluggable abstraction defaulting to OFF**. v1 ships two modes — **DeepL** (recommended cloud provider) and **off**; a **local no-egress engine** (LibreTranslate the likely AGPL-compatible candidate) is future work. Default-off means a self-hoster must make a **conscious, explicit** choice to enable translation; the chosen provider (a GDPR data processor) is disclosed in the self-host env catalog (`docs/operations/SELFHOST_ENV.md`, Contract C21) and the privacy surface.

**Rationale**: feedbackmonk is AGPL and **self-hosted** (GitCellar runs it on its own Railway). A hard external-API call would silently make every self-hoster a data exporter to a third party they never chose — feedback bodies are personal data (free text + stored `end_user_email`/`end_user_name`), so any translation provider is a GDPR **data processor** requiring a DPA + privacy-policy disclosure, and a US provider (Google) needs a cross-border transfer mechanism (SCCs). That collides with DEC-FBR-02's privacy brand promise unless egress is opt-in and disclosed. DeepL is preferred over Google for EU data: EU-based, offers a DPA, and contractually does not train on Pro-API text. The local no-egress engine is the long-term answer for the strictest self-hosters but is deferred to keep v1 scope tight.

**Legal/compliance notes (load-bearing — do not silently relax)**: translation provider = GDPR data processor → DPA + disclosure required; US providers need SCCs; self-hosters become controllers and inherit these obligations — which is *why* the provider must be configurable, default-off, egress disclosed, and a no-egress path eventually available. This is the privacy-egress reasoning behind the default-off posture; an agent must not flip the default to a cloud provider, or remove the off/local options, without re-opening this decision.

**Scope** (planned): provider-abstraction module (trait + DeepL adapter + off no-op), config surface for provider selection + credentials, env-catalog disclosure entry (Contract C21). Per-row storage/processing model is DEC-FBR-IMPL-25.
...
**Decision**: the catalog tree `i18n/locales/<code>/<ns>.json` (Contract C35, per-namespace files as refined at plan time: nested JSON, `_meta.{language,status}`, `{{name}}` interpolation, CLDR plural suffixes `_one/_other/_few/_many/_zero`) is the sole source. Namespaces partition it by consumer: `widget.*`, `public.*`, `admin.*`, `email.*`, shared `status.*`/`kind.*`. Consumers:
