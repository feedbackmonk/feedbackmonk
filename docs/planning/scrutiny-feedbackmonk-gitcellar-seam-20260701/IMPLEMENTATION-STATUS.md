...
| P1-4 combined state+ledger transition primitive | `transition_in_executor` in WorkOrderRepo (writes state + `work_order_events` ledger row in one executor; scope predicate = tenant/project guard; 2 passing tests) | `432ad82` | ✅ DONE (verified 2026-07-03; row was stale — FINAL STATE prose already listed it shipped) |
...
| P2-2 widget footer_url/logo_url scheme allowlist + primary_color validation | ⏳ READY — **blocker root-caused + cleared 2026-07-03**; code patch not yet applied. The prior *"BLOCKED — rollup native-binary bug on Node 22/Windows, persists after clean `npm ci`"* note was a **MISDIAGNOSIS** (it was never npm bug 4828 / rollup). Real cause: the machine-global `~/.npmrc` carries `os=linux`, which forces npm to resolve optional platform binaries as Linux and silently skip `@rollup/rollup-win32-x64-msvc` on this Windows box — so *every* install, including a clean `npm ci`, omits it (which is exactly why "clean reinstall didn't help"). **Fix (verified end-to-end 2026-07-03):** `npm install --os=win32` (CLI overrides the `~/.npmrc` `os=linux`) materializes the win32 binary; `npm run build` then completes clean (vite build + terser minify, `dist/` regenerated). **Ready code fix (unchanged):** `safeHttpUrl()` (allow only http/https, else fallback) on `footer_url`→href + `logo_url`→img.src, and `safeCssColor()` on `primary_color`, in `widget/src/ui.ts` (all three sinks confirmed still unsanitized: `ui.ts:123/254/305`). **Remaining:** apply the patch → `npm run build` (with `--os=win32` on this machine) → re-verify `widget-bundle-size`. NB: this env quirk gates *any* widget build on this box, not just this fix — now understood and one-flag-workable, not infra rot. |
...
- **P2-2 widget `href`/`logo`/`color` scheme allowlist** — patch written + `tsc -b` clean; **not yet applied** (real, still-open finding: `footer_url`→href, `logo_url`→img.src, `primary_color`→CSS var are all unsanitized in `widget/src/ui.ts`; a `javascript:` footer URL is the live vector). **UPDATE 2026-07-03:** the earlier "infra blocker (rollup native-binary bug, persists after clean `npm ci`)" was a **misdiagnosis** — the real cause is the machine-global `~/.npmrc` `os=linux` making npm skip the Windows-native rollup binary. `npm install --os=win32` clears it and `npm run build` completes green (verified). The security patch (`safeHttpUrl`/`safeCssColor` in `widget/src/ui.ts`) is now shippable; only its application + `widget-bundle-size` re-verify remain.
