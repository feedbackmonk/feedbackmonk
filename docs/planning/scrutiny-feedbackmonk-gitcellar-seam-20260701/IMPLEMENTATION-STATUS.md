...
| P2-2 widget footer_url/logo_url scheme allowlist + primary_color validation | ✅ DONE (2026-07-03) — `safeHttpUrl()` (http/https-only, else fallback/skip) guards `footer_url`→href + `logo_url`→img.src, `safeCssColor()` guards `primary_color`→CSS var, in `widget/src/ui.ts`. `dist/` rebuilt; `widget-bundle-size` re-verified PASS (15,041B / 30,720 cap). New regression tests in `e2e/widget-a11y.spec.ts` ("config-value hardening (P2-2)") prove javascript: URLs are dropped + valid values honored; full suite 13/13 green. **The earlier "BLOCKED — rollup native-binary bug, persists after clean `npm ci`" note was a MISDIAGNOSIS** — the real cause was the machine-global `~/.npmrc` `os=linux` making npm skip `@rollup/rollup-win32-x64-msvc` on Windows; cleared with `npm install --os=win32` (documented in `~/.claude/MACHINE_CONFIG.md`). |
...
### Resolved after the fix-wave session
- **P2-2 widget `href`/`logo`/`color` scheme allowlist — ✅ DONE 2026-07-03** (post-session follow-up). `safeHttpUrl()`/`safeCssColor()` guards land in `widget/src/ui.ts` on all three sinks (`footer_url`→href, `logo_url`→img.src, `primary_color`→CSS var); `dist/` rebuilt; `widget-bundle-size` PASS (15,041B); regression tests in `e2e/widget-a11y.spec.ts` prove `javascript:` URLs drop + valid values honored (13/13 green). The prior "infra blocker (rollup native-binary bug, persists after clean `npm ci`)" was a **misdiagnosis** — real cause was the machine-global `~/.npmrc` `os=linux` making npm skip the Windows-native rollup binary; `npm install --os=win32` clears it (now documented in `~/.claude/MACHINE_CONFIG.md`).
