...
- `kind: "verification"`
- `freshness: trigger-invalidate on changes to `migrations/`, `crates/feedbackr-repository/`, or `crates/feedbackr-core/`
- Two probes:
  1. **AST/grep probe over the API + handler crates**: any `sqlx::query`, `sqlx::query_as`, `sqlx::query!`, `pool.acquire()`, or `&mut Connection` reference *outside* `crates/feedbackr-repository/` fails the oracle.
  2. **Repository-method audit**: every public function in `crates/feedbackr-repository/` must take `&TenantScope` or `&ProjectScope` as its first non-`&self` argument, or be marked `#[doc(hidden)] pub(crate)` admin-only with an explicit allow-listed call site.
...
└── Sub-task 1: Data model + tenant-scoped repository layer (FR-FBR-01)
    ...
    ├── Domain types (crates/feedbackr-core/) — Tenant, Project, SigningKey, Feedback, AnonSubmission, RateLimitCounter
    ├── Repository traits + impls (crates/feedbackr-repository/) — TenantScope, ProjectScope, TenantRepo, ProjectRepo, SigningKeyRepo, FeedbackRepo
    └── Oracle wired to CI
...
└── Worker B — Submission Path (FR-FBR-03 + FR-FBR-05 + FR-FBR-06)
    ├── JWT EdDSA verifier (crates/feedbackr-jwt/)
    ...
    ├── Anonymous mode (crates/feedbackr-anon/)
    ...
    └── Submission domain logic in handler (crates/feedbackr-api/handlers/submission.rs)
...
```rust
// crates/feedbackr-repository/src/scope.rs
...
```rust
// crates/feedbackr-repository/src/lib.rs
...
```rust
// crates/feedbackr-jwt/src/lib.rs
...
X-Feedbackr-Anon-Cookie: <opaque-base64>  (optional — anonymous mode dedup)
...
| 12-factor env-var sprawl confuses self-hosters at P4 | Low | Medium | P0 names env vars from a coherent set (`FEEDBACKR_*` prefix) and emits a `.env.example` at end of Stage 3. P4 self-host docs port this verbatim. |
...
5. **Stage 3 trigger** (single session in converging Lead Developer's tree):
   ...
   - `/0-uldf-finalize` once P0 exit gate (curl-able end-to-end + oracle green + /health JSON) is reached. P0 then closes; P1 begins via a fresh `/0-uldf-ldis-plan "Feedbackr P1 — Closes the Loop"` round.
