# Execution Plan — feedbackmonk P1 — Closes the Loop
**Source**: /0-uldf-ldis-plan
**Generated**: 2026-05-13T23:11:15Z
**Task**: Build the P1 (Closes the Loop) slice of feedbackmonk v1 — status workflow state machine + audit history + admin UI (React+Vite) + status emails + PII scrubber with drift-detection oracle. Exit gate: "admin can list feedback, click drawer, transition status (with audit row), reply public + internal; submission → confirmation email → status-change email → public-reply email all observed via Mailpit; PII scrubber active on all server logs and oracle green."
...
**P0 Reference Plan**: `docs/planning/plans/20260513T210133-feedbackmonk-p0-foundation.md` (structural template)
...
- `kind: "verification"`
- `freshness: trigger-invalidate` on changes to `crates/feedbackmonk-tracing/` (new crate Stage 1 creates) or `migrations/`
- Two probes:
  1. **AST/grep probe over all crates**: any direct `tracing_subscriber::fmt()`, `tracing_subscriber::registry()`, or custom `Layer` impl OUTSIDE `crates/feedbackmonk-tracing/` fails the oracle.
  2. **Pattern-set hash probe**: SHA-256 of the canonical 20-pattern list (named regex array in `feedbackmonk-tracing::scrubber::CANONICAL_PATTERNS`) must match a hash recorded in `.claude/oracles/pii-scrub-audit/expected_hash.txt`. Changes require an explicit hash update commit, surfacing pattern-set evolution as a reviewable change.
...
└── Sub-task 1: PII scrubber impl (FR-FBR-10)
    ├── New crate: crates/feedbackmonk-tracing/
    ...
    ├── Wire into bin/feedbackmonk-api/src/main.rs replacing existing tracing setup from P0 Stage 3
...
└── Sub-task 4: Repository surface extensions (Contract C6 backing methods)
    ├── crates/feedbackmonk-repository/src/feedback.rs — add: list_for_admin(scope, status_filter?, limit, offset), get_with_history(scope, feedback_id) -> (Feedback, Vec<StatusHistoryRow>)
    └── crates/feedbackmonk-repository/src/feedback_status_history.rs — new module: FeedbackStatusHistoryRepo trait + SqlxFeedbackStatusHistoryRepo impl
...
│   │   ├── feedbackmonk_core::FeedbackStatus enum: Submitted | Triaged | InProgress | Shipped | WontFix | Duplicate (already in P0 schema; reuse)
...
│   │   └── FeedbackReplyRepo trait + SqlxFeedbackReplyRepo impl in feedbackmonk-repository
│   ├── Status emails (FR-FBR-09)
│   │   ├── crates/feedbackmonk-api/src/email/templates.rs — port from gitcellar-cloud/src/feedback/email_templates.rs
...
│   │   ├── Sender display name: `{tenant.brand_name} via feedbackmonk`
...
└── Sub-task 2: Carry-forward critic C-002
    └── crates/feedbackmonk-api/tests/router_submission_integration.rs (or similar location) — axum-Router-level integration tests for the P0 submission handler
...
└── Sub-task 3: Carry-forward ULADP module READMEs
    └── Author README.md for each of: crates/feedbackmonk-anon/, crates/feedbackmonk-jwt/, crates/feedbackmonk-api/src/auth/, crates/feedbackmonk-api/src/email/, crates/feedbackmonk-api/src/handlers/
...
```rust
// crates/feedbackmonk-core/src/status.rs — extends P0's FeedbackStatus enum
...
Cookie: feedbackmonk_admin_session=<value>  (P0 carry-state)
...
Cookie: feedbackmonk_admin_session=<value>
Content-Type: application/json
...
3. Direct `tracing_subscriber::fmt()` builder calls outside `crates/feedbackmonk-tracing/` fail oracle Probe A.
...
```rust
// crates/feedbackmonk-api/src/email/context.rs
...
- Cookie name: `feedbackmonk_admin_session` (verify in P0 code if exact name differs)
...
**Stage 1 agent action**: read the existing P0 admin-session module (`crates/feedbackmonk-api/src/auth/` per P0 critic note), document the cookie shape verbatim in Stage 1's handoff doc, do NOT re-implement.
...
| Q5 | Drift detection | pattern-set hash + AST probe over all crates; oracle re-runs on any change to `feedbackmonk-tracing/` |

**Composite 11 (borderline); flagged because Q2=4 with scaffolding planned (Q5 mandatory) and Q3 ≥ 3.**

**Recommendation**: build the `pii-scrub-audit` oracle as Task Zero of Stage 1 BEFORE the scrubber implementation lands. Three-leg defense per D-FBR-02 (type system + AST oracle + lint baseline) — the same shape that made `multi-tenant-isolation-check` reliable. Pattern-by-pattern unit tests live in `crates/feedbackmonk-tracing/tests/`. Stage 2 Worker A's new log emissions automatically inherit the scrubbed layer (no per-worker integration cost).
...
|---|---|---|---|
| `feedbackmonk_repository::FeedbackRepo` | **EXTEND** — add `list_for_admin(scope, status_filter?, limit, offset)` and `get_with_history(scope, feedback_id)`. Existing methods unchanged. | Worker A's admin handlers; no P0 consumer breaks. | None — additive. |
| `feedbackmonk_repository::TenantRepo` | **EXTEND** — add `update_brand(scope, EmailTenantBrand)`; brand fields may be fetched via `find_by_email` widening or via a new `get_brand(scope)` method. Stage 1 agent picks. | Stage 1 (Contract C10 wiring); Worker A's email renderers. | None — additive. |
| `crates/feedbackmonk-api/src/main.rs` tracing setup | **REPLACE** — current P0 Stage 3 inline `tracing_subscriber::fmt()` builder is replaced by `feedbackmonk_tracing::install_global_subscriber(...)`. | One file change; verified by existing P0 e2e witness still passing. | Low — single binary file. |
| `feedbackmonk_core::FeedbackStatus` | **NO CHANGE** — enum variants are already in P0 schema; only the state-machine transition rules are NEW (in `feedbackmonk_core::status`). | Worker A consumes via `legal_transitions_from`. | None. |

### New Surfaces

- New crate: `crates/feedbackmonk-tracing/` (Stage 1)
...
| P3 tier display (FR-FBR-14) | "Powered by feedbackmonk" widget footer (P2) + tier-cap counter reads in admin UI dashboard (P3) | C10's EmailTenantBrand + a future EmailTenantTier field can be additive in P3; P1 doesn't anticipate. |
| P4 self-host docs | All EmailTenantBrand parameter names; PII scrubber pattern-set documentation; env-var configuration surface (already 12-factor from P0) | C10 docstrings + `feedbackmonk-tracing::scrubber::CANONICAL_PATTERNS` doc comments + `docs/operations/EMAIL_TEMPLATES.md` (Stage 3 sub-task) |
...
- Worker A touches: `crates/feedbackmonk-api/src/handlers/admin_feedback.rs`, `crates/feedbackmonk-api/src/email/`, `crates/feedbackmonk-repository/src/feedback.rs` (extension), `migrations/00004_feedback_replies.sql`, new repo modules.
- Worker B touches: `admin-ui/` entirely; no Rust crate touches.
- **Zero file conflicts expected.** Worker B's only Rust touches would be `crates/feedbackmonk-api/src/main.rs` if a CORS / static-file-serving change is needed for the admin UI; defer this to Stage 3 e2e integration (orchestrator owns).
...
6. **After P1 close**: fresh `/0-uldf-ldis-plan "feedbackmonk P2 — Customer-Facing"` round consuming this plan as the predecessor + the arc plan's §P2 brief.
...
- **Stage 1 agent**: your deliverable is **(a)** the `pii-scrub-audit` Verification Oracle GREEN, **(b)** the `feedbackmonk-tracing` crate with 20 canonical patterns, **(c)** the `feedback_status_history` migration, **(d)** Contracts C6/C7/C8/C10 frozen and written to `docs/planning/handoffs/p1-stage1-to-stage2.md`, **(e)** repository extensions to `FeedbackRepo` + `TenantRepo` (no new pre-auth allowlist entries needed — these are scope-clean additions), and **(f)** carry-state read of P0's admin session module documented for Worker A/B reference. Resist scope creep into Stage 2 (status transitions, email rendering, admin UI). Contract-first.
