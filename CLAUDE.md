...
- **Trigger: your word — the `feedbackmonk` database has no logical backup of its own** — GitCellar's nightly `gitcellar-pg-backup` cron dumps the `railway` database, not this one, so the only coverage is Railway's project-level point-in-time restore. There is no dump on this machine any more (the one-off pre-migration snapshot was pruned 2026-09-10 once its conditions were met). Deciding whether that is enough is an owner call.
