...
| `public-board-moderation-gate` | Public Board Stage 0 | 🟡 SCAFFOLD (built Stage 0, 2026-06-19) — defends the **moderation trust boundary** for the public feedback board (FR-FBR-25a sibling): no public-board endpoint may return a feedback row whose `moderation_status != approved`, and the board wire shape leaks no submitter PII (DEC-FBR-02 / Q24 class). Detection-from-code (parses `moderation.rs` `is_publicly_visible` + asserts the board read SQL hard-filters `approved` + scans the wire shape for PII), NOT a self-reported flag — the anti-reward-hacking leg. Probe A (state-machine source) **live**; B (board read approved-only + no-PII) + C (`--full`: `tests/board_moderation_gate.rs` + `board_privacy_isolation.rs`) **PENDING** until Stage 1 Worker A lands the board path. Cold-start PASS. Becomes the Stage 1 exit gate (GATE 1). |
