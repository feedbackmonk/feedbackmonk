...
- **Local Postgres dev container**: `feedbackr-pg-dev` on port 5433 (`DATABASE_URL=postgres://postgres:dev@localhost:5433/feedbackmonk_dev`). Used for `cargo sqlx prepare`. Container name still has `feedbackr-` prefix (not yet renamed; orthogonal to PF-RENAME-02).
