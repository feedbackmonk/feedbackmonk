# Task Queue — feedbackmonk P0 Foundation
...
- [ ] Build `multi-tenant-isolation-check` Verification Oracle in `.claude/oracles/`
  - Skeleton + freshness contract (trigger-invalidate on migrations/, crates/feedbackmonk-repository/, crates/feedbackmonk-core/)
...
- [ ] Cargo workspace skeleton: `crates/feedbackmonk-core`, `crates/feedbackmonk-repository`, `crates/feedbackmonk-api` (placeholder), `migrations/`
...
- [ ] `TenantScope` / `ProjectScope` newtypes in `crates/feedbackmonk-repository/src/scope.rs` with `pub(crate)` constructors; `open()` as the SOLE `ProjectScope` constructor
- [ ] Repository traits + impls in `crates/feedbackmonk-repository/src/`: `TenantRepo`, `ProjectRepo`, `SigningKeyRepo`, `FeedbackRepo` — every method takes `&TenantScope` or `&ProjectScope` as first non-`&self` arg
...
- [ ] Backend dev port `14304` claimed in `~/.claude/MACHINE_CONFIG.md` Dev Port Registry; feedbackmonk row's API column updated
