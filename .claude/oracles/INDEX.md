...
| **`feedback-erasure-completeness`** | Does the Phase A A1 right-to-erasure path (`DELETE …/me/feedback/{id}`) erase ALL of a submission with no residual PII? Probe A proves from `me_feedback.rs::delete_my_feedback` that attachment OBJECT BYTES are purged (`ObjectStore::delete` over `list_storage_keys_for_feedback`) BEFORE the feedback ROW delete (DB cascade removes only metadata rows, never bucket bytes); Probe B proves every table that `REFERENCES feedback(id)` is `ON DELETE CASCADE`/`SET NULL` (never NO ACTION — which would block erasure or orphan child rows); Probe C proves the erasure `DELETE` (`delete_for_end_user`) scopes its WHERE by `end_user_sub` + `tenant_id` + `project_id` (no cross-user erasure hole); **Probe D (scrutiny P0-1)** proves `delete_for_end_user` also UPDATE-scrubs the P5 analysis corpus (`feedback_clusters`/`recommendations`/`work_orders`/`analysis_sweeps`) for the erased feedback's cluster — those tables reference the CLUSTER not feedback, so FK cascade never reaches them and the submitter's verbatim/paraphrased text would otherwise survive in admin-readable prose. With `--full`: runs `tests/me_feedback_delete.rs` (incl. `delete_scrubs_p5_derived_text`). Detection-from-code (anti-reward-hacking leg). *(Phase A A1 / D-A1; scrutiny P0-1; DEC-FBR-04 / DEC-FBR-03; DEC-FBR-02 / Q24)* | verification | trigger-invalidate (`crates/feedbackmonk-api/src/handlers/me_feedback.rs`, `crates/feedbackmonk-repository/src/feedback.rs`, `crates/feedbackmonk-repository/src/attachments.rs`, `crates/feedbackmonk-api/src/storage.rs`, `crates/feedbackmonk-api/tests/me_feedback_delete.rs`, `migrations`, oracle.py) | me_feedback delete path + erasure query + derived-text scrub + attachment purge + feedback-child migrations (every commit) | ~900 tokens |
| **`public-route-ceiling`** | Is every public router (`submission`/`attachments`/`board`/`roadmap`) in `build_app` wrapped by `apply_public_rate_limit` (the class-level per-IP ceiling)? Anti-treadmill guard for P0-2 — the abuse floor moved from a per-handler cookie-keyed limiter (which every post-P0 route silently failed to inherit) to router middleware; this catches a future public route added without the ceiling (the wiring-removal class, like `cors-allowlist-enforcement`). Detection-from-code. *(scrutiny 2026-07-01 Arc 1; P0-2)* | verification | trigger-invalidate (`crates/feedbackmonk-api/src/main.rs`, oracle.py) | build_app public-router wiring (every commit) | ~500 tokens |
| **`public-id-as-capability`** | Do attachment list/download bind reads to the SUBMITTER (`authorize_submitter` before any data call, checking `owner_of` + JWT `sub` + anon cookie hash)? Anti-treadmill guard for P0-3 — enforces "a public identifier is never an access token" so a leaked short code cannot re-become a bearer capability that enumerates/serves another user's PII attachments. Detection-from-code. *(scrutiny 2026-07-01 Arc 1; P0-3)* | verification | trigger-invalidate (`crates/feedbackmonk-api/src/handlers/attachments.rs`, oracle.py) | attachment read authz (every commit) | ~600 tokens |
| **`submission-idempotency`** | Is the submit `Idempotency-Key` dedupe identity-scoped (PK includes `submitter_id` + a key-length CHECK) and content-fingerprinted (`claim_idempotency_key` writes `content_hash` and returns `IdempotencyKeyReuse` on mismatch)? Anti-reward-hacking static leg for P1-3 — a shared key across users must not drop the second user's feedback, and a key reused with different content must 409 not silently return the original. Detection-from-code. *(scrutiny 2026-07-01 Arc 1; P1-3 / M6 / P2-17)* | verification | trigger-invalidate (`migrations/**`, `crates/feedbackmonk-repository/src/feedback.rs`, oracle.py) | submit_idempotency PK + claim_idempotency_key (every commit) | ~600 tokens |
...
| Oracle | Question | Kind | Strategy | Consumer Scope | Est. savings/call |
|---|---|---|---|---|---|
| **`widget-bundle-size`** | Is the ENGLISH PAGE-LOAD SET of the built widget (TOP-LEVEL `widget/dist/*.{js,mjs,css}`) at most 30720 bytes (30 KiB; FR-FBR-04 cap), is every per-locale catalog chunk (`widget/dist/locales/*.js`) at most 4096 bytes (C42), and does the built tree contain zero canonical third-party tracker hostnames anywhere? Has the canonical tracker-list drifted from its hashed baseline? *(P2 Task Zero — CLAUDE-A worker, collab-20260514-035703; **v1.1.0 amended for FR-FBR-35 localization, collab-20260906-215851**: Probe A measures the top-level page-load set instead of a recursive sum — 31 mutually-exclusive locale chunks made "everything under dist/" a page load nobody performs — and new Probe C caps each chunk individually; `SIZE_CAP_BYTES` UNCHANGED at 30720, Probe B still recurses. Three-leg defense leg 2 — paired with vite.config.ts terser+CSP-safe bundler chokepoint (leg 1) + Playwright+axe-core a11y harness (leg 3); enforces FR-FBR-04 size cap + DEC-FBR-02 no-third-party-trackers brand promise; cold-start vacuous-PASS supports Task Zero order-of-operations)* | verification | trigger-invalidate (`widget/dist/**`, `widget/src/**`, `widget/scripts/**`, `widget/vite.config.ts`, `widget/package.json`, `i18n/locales/*/widget.json`, `expected-trackers.txt`) | P2+ widget/ (every commit touching widget/dist, widget/src or the widget catalog; CI gate) | ~800 tokens |
...
Invocation: `bash .claude/oracles/i18n-catalog-integrity/oracle.sh` (Unix) or `pwsh .claude/oracles/i18n-catalog-integrity/oracle.ps1` (Windows); same shape for the other two. **Stage 1 state (2026-09-06)**: all three implemented by worker W-T. `i18n-catalog-integrity` Probes A-E all live (Probes C/D/E delegate to `scripts/i18n/validate.py`'s own defect classes; adversarially self-tested — see its README; manifest at 1.0.0 (LD-granted, MSG-008)). `i18n-literal-ratchet` runs its real scanner + `--freeze`; vacuous-PASS until the LD freezes `i18n/literal-baseline.json` at Stage-1 converge. `translation-gap-status` wraps `scripts/i18n/check-gaps.py --json` and prints the schema line above. All three ship an invertible `--self-test`.
