...
Invocation: `pwsh .claude/oracles/multi-tenant-isolation-check/oracle.ps1` (Windows) or `bash .claude/oracles/multi-tenant-isolation-check/oracle.sh` (Unix); `python .claude/oracles/pii-scrub-audit/oracle.py` (cross-platform) or `bash .claude/oracles/pii-scrub-audit/oracle.sh` (Unix shim). CORS: `bash .claude/oracles/cors-allowlist-enforcement/oracle.sh` (Unix; `--full` runs Probe C) or `pwsh .claude/oracles/cors-allowlist-enforcement/oracle.ps1` (Windows); `python .claude/oracles/cors-allowlist-enforcement/oracle.py [--full]` (cross-platform direct).
